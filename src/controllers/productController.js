...
exports.createProduct = async (req, res) => {
  try {
    ...
    // Validate stock: at least one of stockQty or stockGrams must be provided
    if (stockQty === undefined && stockGrams === undefined) {
      return res
        .status(400)
        .json({ message: "Please provide either stockQty or stockGrams" });
    }
    ...
    const created = await Product.create({
      ...
      stockGrams: stockGrams !== undefined ? Number(stockGrams) : undefined,
      category: catId,
    ...
    const populated = await Product.findById(created._id)
      .populate({ path: "category", select: "name isActive" })
      .populate({ path: "subcategory", select: "name isActive category" })
      .select('name price stockQty stockGrams unit category subcategory isActive') // Add stockGrams
      .lean();
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    const updated = await Product.findByIdAndUpdate(
      ...
      .populate({ path: "category", select: "name isActive" })
      .populate({ path: "subcategory", select: "name isActive category" })
      .select('name price stockQty stockGrams unit category subcategory isActive')
      .lean();
